...
// 獲取所有 class 為 "special" 的 input 元素
const SpecialInputs = document.querySelectorAll('input.special');
...
// 計算特殊關卡的分數
function SpecialTotalScore() {
    specialLevel_score = 0;
    // 遍歷所有輸入框
    SpecialInputs.forEach(input => {
        const special_id = input.id; // 獲取輸入框的 id
        console.log("id+" + special_id)
        const value = parseFloat(input.value) || 0; // 獲取輸入框的值，若為空則默認為 0
        const SpecialLevel = specialLevel[special_id] || 0; // 獲取該關卡的基礎分數
        console.log("value:" + value + "特殊關卡基礎分:" + SpecialLevel);
        // 計算該關卡的分數

        specialLevel_score += value * SpecialLevel;
        console.log("新增" + specialLevel_score + "分");
    });
...
threeFBossCheckboxes.forEach(checkbox => {
    checkbox.addEventListener('change', (event) => {
        ...
        if (isChecked) {
            console.log(`Checkbox ${id} 被選中，分數增加: ${value}`);
            threeFBoss += value;
            console.log("增加boss分數");
            comtestscore();
        } else {
            // 如果 checkbox 被取消選中，執行相應的操作
            console.log(`Checkbox ${id} 被取消選中，分數減少: ${value}`,);
            threeFBoss -= value;
            console.log("減少boss分數");
            comtestscore();
